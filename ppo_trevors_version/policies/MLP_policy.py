...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        distro_from_MLP= self.forward(observation=observation)

        if self.discrete:
            action_from_MLP = distro_from_MLP.sample()
        else:
            action_from_MLP = distro_from_MLP.sample()#can I use mean here?

        return ptu.to_numpy(action_from_MLP)
    ...
    def forward(self, observation: torch.FloatTensor):
        ...
        if self.discrete != True:
            batch_mean_from_nn= self.mean_net(observation)
            ...
            logits_from_nn= self.logits_na(observation)
            action_distro_out= distributions.Categorical(logits=logits_from_nn)
        return action_distro_out
